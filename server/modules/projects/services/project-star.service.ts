...
export function applyLegacyStarredProjectIds(projectIds: string[], userId: number): ApplyLegacyStarredProjectIdsResult {
  ...
  for (const projectId of normalizedProjectIds) {
    const project = projectsDb.getProjectById(projectId, userId);
    if (!project) {
      continue;
    }
    ...
    projectsDb.updateProjectIsStarredById(projectId, true, userId);
    updated += 1;
  }
...
export function toggleProjectStar(
  ...
  const project = projectsDb.getProjectById(normalizedProjectId, userId, scopeAll);
  if (!project) {
  ...
  projectsDb.updateProjectIsStarredById(normalizedProjectId, nextStarredState, userId, scopeAll);
